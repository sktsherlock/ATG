...
def main():
    ...
    encoder = MLP(PLM_feat.shape[1], n_classes, args.n_layers, args.n_hidden, F.relu, args.dropout).to(device)

    model = LPMLP(encoder, LLM_feat.shape[1], PLM_feat.shape[1], args.alpha, args.conv_type).to(device)
    TRAIN_NUMBERS = sum(
