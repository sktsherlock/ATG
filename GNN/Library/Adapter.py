...
def args_init():
    ...
    argparser.add_argument(
        "--teacher_edge_drop", type=float, default=0.0, help="dropout rate"
    )
    argparser.add_argument(
...
def main():
    ...
    if args.teacher_name == 'RevGAT':
        teacher_model = RevGAT(feat.shape[1], n_classes, args.teacher_n_hidden, args.teacher_layers, args.teacher_n_heads,
                               ...
                               edge_drop=args.teacher_edge_drop, use_attn_dst=False, use_symmetric_norm=True).to(device)
    ...
    teacher_file_prefix = f"lr_{args.teacher_lr}_d_{args.teacher_drop}_e_{args.teacher_edge_drop}_h_{args.teacher_n_hidden}_l_{args.teacher_layers}_h_{args.teacher_n_heads}"
    # 保存 teacher model
