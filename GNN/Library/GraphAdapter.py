...
class GraphClassifier(nn.Module):
    ...
    def reset_parameters(self):
        self.GraphTeacher.reset_parameters()

        self.classifier.reset_parameters()

    def forward(self, feat, graph):
        # Extract outputs from the model
        outputs = self.GraphTeacher(feat, graph)
...
def main():
    ...
    teacher_model = GraphClassifier(GraphTeacher, in_feats=args.teacher_n_hidden, n_labels=n_classes).to(device)

    TRAIN_NUMBERS = sum(
        [np.prod(p.size()) for p in teacher_model.parameters() if p.requires_grad]
    )
    ALL_NUMBERS = sum([np.prod(p.size()) for p in teacher_model.parameters()])

    print(f"Number of the Teacher model Training Params: {TRAIN_NUMBERS}")
    print(f"Number of the Teacher model Params: {ALL_NUMBERS}")

    TRAIN_NUMBERS = sum(
        [np.prod(p.size()) for p in student_model.parameters() if p.requires_grad]
    )
    ALL_NUMBERS = sum([np.prod(p.size()) for p in student_model.parameters()])

    print(f"Number of the Student model Training params: {TRAIN_NUMBERS}")
    print(f"Number of the Student model Params: {ALL_NUMBERS}")
    ...
    if args.save:
        ...
        filename = os.path.join(student_save_path, f"GraphLearner_{student_file_prefix}.pkl")
